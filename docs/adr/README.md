# 設計記録（ADR）

このプロジェクトの設計上の決定の記録（Architecture Decision Record）。
何を決めたかと、なぜそう決めたかを、決定の時点で短く残す。

## 記録する基準

次の 3 つが揃った決定だけを記録する:

1. **覆す・蒸し返すコストが実際に大きい** — 覆すと実装のやり直しが要る、
   または記録が無いと同じ検討・議論が繰り返される
2. **文脈なしでは驚きがある** — 将来の読み手がコードを見て「なぜこうした？」と思う
3. **実際のトレードオフの結果** — 本物の代替案があり、理由をもって選んだ

揃わなければ記録しない。再検討が安い決定（コードを見れば自明で、誰がやり直しても
同じ結論に至る）はただ再決定すればよい。記録に値する典型は
**却下の理由が非自明な代替案** — 書かないと後で誰かが同じ案をまた提案してくる。

## 運用

- 新しい決定は連番ファイル（`NNNN-短いスラッグ.md`）でこのディレクトリに追加し、
  下の索引に 1 行足す
- 過去の決定を覆すときは新しい ADR を起こし、古い方のステータスを
  「廃止（NNNN により置換）」に変更する（ファイルは消さない）

## 形式

タイトル + 1〜3 文で十分。価値は「決定があったこと」と「なぜか」の記録にあり、
セクションを埋めることにはない。

```md
# {決定の短いタイトル}

{1〜3 文: どんな文脈で、何を決め、なぜか。これだけで完結してよい。}
```

本当に価値があるときだけセクションを足す。育てても形が揃うよう、語彙は以下に固定する:

- 冒頭のリスト行: `- ステータス: 承認済み`（見直されうる決定のとき。廃止時は
  `- ステータス: 廃止（NNNN により置換）`）、`- 日付: YYYY-MM-DD`
- `## 検討した代替案` — 却下の理由が非自明で、再提案を防ぎたいとき
- `## 結果` — 非自明な波及効果を明記したいとき
- 本文が長く育ったら `## コンテキスト` と `## 決定` に分ける

## 索引

| ADR | タイトル | ステータス |
|---|---|---|
| [0001](0001-migrations-in-code.md) | マイグレーションはコード内に持ち、`diet init`で適用する | 承認済み（一部を0012が置換） |
| [0002](0002-profile-schema-as-source-of-truth.md) | プロフィールの形式はJSON Schemaを正本にし、検証は自作する | 承認済み |
| [0003](0003-cap-calorie-deficit.md) | 摂取目標へ適用する赤字を暫定維持カロリーの25%までに制限する | 承認済み |
| [0004](0004-routine-as-ordered-steps.md) | ルーティーンは時刻でなく順序で持ち、照合はエージェントが行う | 承認済み |
| [0005](0005-report-day-start-time.md) | レポート日はプロフィールの一日開始時刻から導出する | 承認済み |
| [0006](0006-html-reports-as-static-files-with-chartjs.md) | HTMLレポートはChart.jsを埋め込んだ単一の静的ファイルとして生成する | 承認済み |
| [0007](0007-unrecorded-values-are-missing.md) | 未記録の値をゼロとして集計しない | 承認済み |
| [0008](0008-report-design-language.md) | HTMLレポートは面ではなく罫線で構造を作り、系列色は実測と参照の2色に固定する | 承認済み |
| [0009](0009-goal-deletion-is-logical.md) | 目標の削除は論理削除にし、planは残す | 承認済み |
| [0010](0010-advice-history-keeps-latest-only.md) | 助言は種別・期間ごとに最新1件だけを保持する | 承認済み |
| [0011](0011-agent-read-only-profile-fields.md) | プロフィールにはエージェントだけが読む項目を置き、読む手順とセットで足す | 承認済み（一部を0013が置換） |
| [0012](0012-migrations-verified-by-test-not-mirrored-files.md) | マイグレーションの記録は`migrations/*.sql`をやめ、スキーマ一致テストで保証する | 承認済み |
| [0013](0013-agent-writes-advice-cli-owns-findings.md) | 助言の文面はエージェントが書き、CLIは分析結果（findings）だけを正本として持つ | 承認済み |
| [0014](0014-nutrient-targets-from-dietary-reference-intakes.md) | 栄養素の目安は食事摂取基準からコードで導出し、カロリー超過中は増量方向の助言を出さない | 承認済み |
