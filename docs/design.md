...
- カロリー目標は基礎代謝情報が不足するMVPでは自動断定しない。減量に必要な理論上の赤字とペースだけを計算する。
...
MVPは手動・JSON入力、inbox取り込み、CRUD、目標ペース、集計、定型助言、レポート、バックアップ、写真整理を扱う。画像認識API、食品データベース連携、代謝量に基づく個別カロリー算出、YAML読込、Web/モバイルUI、クラウド同期は将来拡張とする。
