...
SQLiteを正本に、食事・運動・体重・目標を長期間記録し、AIエージェントとCLIから傾向分析やフィードバックを得る個人向けツールです。Webアプリではなく、ローカルでデータを管理したい1人の利用者を対象にしています。
...
記録日時はそのまま保持され、日次・週次レポートとフィードバックでは7月22日分として集計されます。
...
このプロジェクトでは、記録や分析結果をもとにエージェントが返す短い評価と次の行動を
「フィードバック」と呼びます。提示するタイミングと内容は次の3種類です。

| タイミング | 内容 |
| --- | --- |
| 食事の記録後 | 登録内容と推定上の仮定、その日の摂取カロリー・残りカロリー・残り食数、栄養素の目安との差を示し、次の行動を1つだけ伝える |
| 日次レポートの依頼時 | その日の食事・栄養素・運動・体重・目標達成状況を分析し、最優先のfindingに基づく行動を1つ伝える |
| 週次レポートの依頼時 | 直近7日間の平均と前週差、運動・体重・記録日数を分析し、最優先の行動、代替案、続けたい点、計画変更の要否を伝える |

いずれもユーザーの食事記録またはレポート依頼を契機に提示します。時刻による自動通知はなく、
運動や体重を記録した直後の専用フィードバックもありません。記録が不足して判断できない場合は、
...
保存したフィードバックはレポートに埋め込まれます。未保存のうちはフィードバックが未記載であることと、分析結果の数値だけが表示されます。1日の超過を翌日の極端な制限で相殺するような提案はしません。行動変更は原則1つに絞って提示されます。
