...
画像認識API、医療診断、治療、厳密な栄養・代謝計算、複数ユーザー、複数端末からの同時更新は対象外です。詳細な判断と仮定は[設計概要](docs/design.md)、保存内容は[データベーステーブル一覧](docs/database.md)を参照してください。
...
`dietary_restrictions`・`allergies`・`advice_preference`は`routine`と同じく、CLIのコードではなくエージェントだけが読む項目です。食事を記録するとき、エージェントは推定した品目を食事制限とアレルギーに照合し、該当しそうなものがあれば記録はそのまま行ったうえで「衣に小麦が含まれている可能性があります」のように一言添えます（写真やメニュー名からは断定できないためです）。症状や対処には踏み込みません。`advice_preference`（例: 「まずは継続可能な変更を優先」）は、エージェントが書く助言やコメントの方針です。助言の文面はエージェントが書くので、この方針が反映されます。CLIが計算した数値（レポートの集計と`findings`）は書き換えません。
...
食事登録後は、その日の摂取目標に対する残量、残り食数、栄養素の目安との差を返します。日次は食事・推定範囲・栄養素と目安の差・運動・体重・分析結果・達成判定を、週次は7日平均・前週差・運動・体重・データ不足・分析結果を出します。既定ではMarkdownが`reports/daily/`と`reports/weekly/`に残ります（Git管理外）。

栄養素は摂取量だけでなく目安との差を併記します（例: 「たんぱく質: 52.0 g（目安 53.6〜82.5 g / −1.6）」）。目安は「日本人の食事摂取基準」の目標量から、プロフィールの性別・年齢と目標カロリーをもとに算出します。記録のない栄養素はゼロとして扱わず「記録なし」と表示し、不足の指摘もしません。
...
助言の文面はエージェントが書きます。CLIは分析結果（findings）だけを計算し、文面を作りません。

```bash
diet advice today     # findings（分析結果）を返す
diet advice weekly
diet advice save --json advice.json --kind period --days 7   # 書いた助言を保存する
```

各findingは実測値・参照値・参照根拠・根拠にした日数を持ちます。順位はカロリー管理を栄養素より優先し（`goal`→`calorie`→`nutrition`）、摂取が目標を超えているときは栄養素の不足を追加摂取ではなく置き換えとして扱います。栄養素を満たすために食べ足す助言は生成されません。

保存した助言はレポートに埋め込まれます。未保存のうちは「助言は未記載」と表示され、分析結果の数値だけが載ります。1日の超過を翌日の極端な制限で相殺するような提案はしません。行動変更は原則1つに絞って提示されます。
