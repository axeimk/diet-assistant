-- diet-assistant の初期スキーマ（PRAGMA user_version = 1）。
-- first commit 4784196 の src/diet_assistant/db.py の SCHEMA_SQL をそのまま写したもの。
-- 歴史的事実なので二度と変更しない。tests/test_core.py がマイグレーションの入力として読む。
